...
GameSchema.virtual('isWaiting').get(function () {
...
GameSchema.methods.getPropertyByPosition = function (position) {
    return this.boardState.find((p) => p.position === position);
};
GameSchema.methods.getPlayerProperties = function (playerId) {
    return this.boardState.filter((p) => p.ownerId === playerId);
};
// Pre-save middleware
