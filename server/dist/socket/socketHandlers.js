...
import { getRandomChanceCard, getRandomCommunityChestCard } from '../utils/debuggingCards.js';
...
async function endPlayerTurn(game, gameId, io) {
...
export function setupSocketHandlers(io, socket) {
    ...
    socket.on('buy-property', async ({ gameId, playerId, propertyId, code, language }) => {
        try {
            ...
            const property = game.boardState.find((p) => p.id === propertyId);
...
async function handleDuelEnd(game, io) {
    const duel = game.activeDuel;
    const property = game.boardState.find((p) => p.id === duel.propertyId);
    const challenger = game.players.find((p) => p.id === duel.challengerId);
    const defender = game.players.find((p) => p.id === duel.defenderId);
...
function applyCardEffect(game, player, card, io, gameId) {
