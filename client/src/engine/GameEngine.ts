...
export class GameEngine {
  ...
  buyProperty(playerId: string, propertyId: string): boolean {
    ...
    // Use price field, fallback to cost for backward compatibility
    const propertyCost = (property as any).price || property.cost;
  ...
  payRent(playerId: string, propertyId: string): boolean {
    ...
    // Clamp money to 0 and handle bankruptcy
    if (player.money <= 0) {
      player.money = 0;
